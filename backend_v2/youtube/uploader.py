...
class YouTubeUploader:
    ...
    def upload_video(
        ...
        try:
            ...
            # Final title check for 100 char limit (YouTube requirement)
            if len(title) > 100:
                logger.warning(f"Title too long ({len(title)} chars), truncating for YouTube limit")
                title = self.truncate_title_for_youtube(title, max_length=100)
